...
public class Graph
{
	...
	public int augmentedNodesSize()
	{
		return augmentedNodes.size();
	}
	...
	public void createAugmentedMDP(boolean reverse, int threshold)
	{
		...
		if(reverse)
		{
			...
			for(Integer key : keys)
			{
				...
				AugmentedNode terminalNode = nodes.get(key).extendNode(0);
	...
	public void print(int threshold)
	{
		...
		System.out.println("QTD: " + augmentedNodes.size() + " " + count);
		
	}
	
	public void printAugmented()
	{
		Set<Integer> keys = augmentedNodes.keySet();
	
		for(int i = 0; i <= 96; i++)
		{
			for(Integer key : keys)
			{
				AugmentedNode n = (AugmentedNode)augmentedNodes.get(key);
				int t = n.getThreshold();
				if(i == t) n.print();
			}	
		}
		System.out.println(augmentedNodes.size());
	}
}
