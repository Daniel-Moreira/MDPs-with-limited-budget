...
public class NavigationGen_2
{
	private static final String NUMBER_REGEX = "^[0-9]+$";
	private static final String DOUBLE_REGEX = "^[0-9]+\\.[0-9]+$";
	private static final String VARIABLE_MASK = "x%dy%d"; //e.g. x1y1

	private static final String UNCERTAIN_AND_IMPRECISE_MOVEMENT_TRANSITION = "%1$s (%2$s)"; //e.g. x1y3 (x1y3 ([1.0]) (x2y3 ([0.0]) (x1y2 ([0.9*p1]) ([0.0]))))
	private static final String UNCERTAIN_AND_IMPRECISE_MOVEMENT_TRANSITION2 = "%1$s (%2$s)"; //e.g. x1y3 (x1y3 ([1.0]) (x2y3 ([0.0]) (x1y2 ([0.9*p1]) ([0.0]))))
	private static final String UNCERTAIN_MOVEMENT_TRANSITION = "%1$s (%2$s)"; //e.g. x1y3 (x1y3 ([1.0]) (x2y3 ([0.0]) (x1y2 ([1.0]) ([0.0]))))
	private static final String UNCERTAIN_MOVEMENT_TRANSITION2 = "%1$s (%2$s)"; //e.g. x1y3 (x1y3 ([1.0]) (x2y3 ([0.0]) (x1y2 ([1.0]) ([0.0]))))
	private static final String MOVEMENT_TRANSITION = "%1$s (%2$s)"; //e.g. x2y3 (x2y3 ([1.0]) (x2y2 ([1.0]) ([0.0]) )) 

	private static final String ABSORVENT_TRANSITION = "%1$s (%2$s)"; //e.g. x2y3 (x2y3 ([1.00]) ([0.00]))
	private static final String UNCERTAIN_ABSORVENT_TRANSITION = "%1$s (%1$s ([p%2$d]) ([0.00]))"; //e.g. x2y3 (x2y3 ([1.00]) ([0.00]))
	private static final String NO_TRANSITION = "%1$s ([0.00])"; //e.g. x1y3 ([0.00])
	
	private static final String NO_DEPENDENCY = "%1$s ([%2$f]) (%3$s)"; //e.g. x1y3 ([0.00])
	private static final String NO_DEPENDENCY2 = "%1$s (%2$s) (%3$s)"; //e.g. x1y3 ([0.00])
	...
	private static String generateGoalState(Integer numberOfLines, Integer numberOfColumns) {
		...
		return String.format("(%s ([1.0]) ([0.0]))", variable);
	}
	...
	private static String getMovementTransitionString(String firstVariable, String secondVariable) {
		return String.format(MOVEMENT_TRANSITION, firstVariable, getAllOtherVariables2(firstVariable, secondVariable, 1.0, 1.0));
	}
	...
	private static String getAbsorventTransitionString(String currentVariable, int column, int numberOfColumns, boolean computeExistenceProbability) {
		...
		if (computeExistenceProbability) { //linear formula to compute the probability of movement fail		
			...
			return String.format(ABSORVENT_TRANSITION, currentVariable, getAllOtherVariables(currentVariable, 1.0));
		}
	}
	...
	private static String getUncertainMovementTransitionString(String currentVariable, String previousVariable, String goalVariable, double stayInPlaceProbability, int column, int numberOfColumns, boolean computeExistenceProbability, Double minProbOfDisappear, Double maxProbOfDisappear) {		
		...
		if (computeExistenceProbability) { //linear formula to compute the probability of movement fail
			...
			if(stayInPlaceProbability == 0.0) return String.format(UNCERTAIN_AND_IMPRECISE_MOVEMENT_TRANSITION2, 
			//return String.format(UNCERTAIN_MOVEMENT_TRANSITION,
				currentVariable, getAllOtherVariables(previousVariable, existenceProbability), column);
			return String.format(UNCERTAIN_AND_IMPRECISE_MOVEMENT_TRANSITION, 
			//return String.format(UNCERTAIN_MOVEMENT_TRANSITION,
				currentVariable, getAllOtherVariables2(currentVariable, previousVariable, stayInPlaceProbability, existenceProbability));
		}
		else {
			if(stayInPlaceProbability == 0.0)
				return String.format(UNCERTAIN_MOVEMENT_TRANSITION2, currentVariable, getAllOtherVariables(previousVariable, 1.0));
			return String.format(UNCERTAIN_MOVEMENT_TRANSITION, 
					currentVariable, getAllOtherVariables2(currentVariable, previousVariable, stayInPlaceProbability, 1.0));
		}
	} 
	...
	private static String getAllOtherVariables(String currentVariable, double value)
	{
		int x = Character.getNumericValue(currentVariable.charAt(1));
		int y = Character.getNumericValue(currentVariable.charAt(3));
		
		String result = NO_DEPENDENCY;
		for (int i = 1; i <= nX; i++) 
		{
			for (int j = 1; j <= nY; j++)
			{
				String var = String.format(VARIABLE_MASK, i, j);
				
				if(i != x || j != y)
					result = String.format(result, var, 0.0, NO_DEPENDENCY);
			}
		}
		result = String.format(result, currentVariable, value, "[0.0]");
		
		return result;
	}
	
	private static String getAllOtherVariables2(String currentVariable, String currentVariable2, double value, double value2)
	{
		int x = Character.getNumericValue(currentVariable.charAt(1));
		int x2 = Character.getNumericValue(currentVariable2.charAt(1));
		int y2 = Character.getNumericValue(currentVariable2.charAt(3));
		int y = Character.getNumericValue(currentVariable.charAt(3));
		
		String result = NO_DEPENDENCY2;
		for (int i = 1; i <= nX; i++) 
		{
			for (int j = 1; j <= nY; j++)
			{
				...
				if((i != x || j != y) && (i != x2 || j != y2))
					result = String.format(result, var, "[0.0]", NO_DEPENDENCY2);
			}
		}
		
		String trueSide = String.format("%1$s ([0.0]) ([%2$f])", currentVariable2, value);
		String falseSide = String.format("%1$s ([%2$f]) ([0.0])", currentVariable2, value2);
		result = String.format(result, currentVariable, trueSide, falseSide);
