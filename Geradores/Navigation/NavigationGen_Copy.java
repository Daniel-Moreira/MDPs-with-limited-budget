...
public class NavigationGen_Copy
{
	private static final String NUMBER_REGEX = "^[0-9]+$";
	private static final String DOUBLE_REGEX = "^[0-9]+\\.[0-9]+$";
	private static final String VARIABLE_MASK = "x%dy%d"; //e.g. x1y1

	private static final String UNCERTAIN_AND_IMPRECISE_MOVEMENT_TRANSITION = "%1$s (%1$s (%3$s ([0.0]) ([%4$f])) (%3$s ([%5$f]) ([0.0])))"; //e.g. x1y3 (x1y3 ([1.0]) (x2y3 ([0.0]) (x1y2 ([0.9*p1]) ([0.0]))))
	private static final String UNCERTAIN_AND_IMPRECISE_MOVEMENT_TRANSITION2 = "%1$s (%3$s ([%5$f]) ([0.0]))"; //e.g. x1y3 (x1y3 ([1.0]) (x2y3 ([0.0]) (x1y2 ([0.9*p1]) ([0.0]))))
	private static final String UNCERTAIN_MOVEMENT_TRANSITION = "%1$s (%1$s (%3$s ([0.0]) ([%4$f])) (%3$s ([%5$f]) ([0.0])))"; //e.g. x1y3 (x1y3 ([1.0]) (x2y3 ([0.0]) (x1y2 ([1.0]) ([0.0]))))
	private static final String UNCERTAIN_MOVEMENT_TRANSITION2 = "%1$s (%3$s ([%5$f]) ([0.0]))"; //e.g. x1y3 (x1y3 ([1.0]) (x2y3 ([0.0]) (x1y2 ([1.0]) ([0.0]))))
	private static final String MOVEMENT_TRANSITION = "%1$s (%1$s (%2$s ([0.0]) ([%3$f])) (%2$s ([%4$f]) ([0.0])))"; //e.g. x2y3 (x2y3 ([1.0]) (x2y2 ([1.0]) ([0.0]) )) 
	...
	private static String getUncertainMovementTransitionString(String currentVariable, String previousVariable, String goalVariable, double stayInPlaceProbability, int column, int numberOfColumns, boolean computeExistenceProbability, Double minProbOfDisappear, Double maxProbOfDisappear) {		
		...
		if (computeExistenceProbability) { //linear formula to compute the probability of movement fail
			...
			if(stayInPlaceProbability == 0.0) return String.format(UNCERTAIN_MOVEMENT_TRANSITION2, currentVariable, goalVariable, previousVariable, stayInPlaceProbability, 1.0);
			return String.format(UNCERTAIN_MOVEMENT_TRANSITION, 
